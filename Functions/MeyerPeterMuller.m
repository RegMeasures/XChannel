...
ThetaCrit_i = Inputs.Opt.ST.ThetaCrit * HidExp;

Delta = (Inputs.Sed.Rho_S-Inputs.Hyd.Rho_W)/Inputs.Hyd.Rho_W;
            
qsiTot_flow = Cell.Fi * ...
              Inputs.Opt.ST.MPMcoef .* (ones(Cell.NCells,1)*Frac.Di_m) .* ...
              sqrt(Delta * Inputs.Hyd.g * (ones(Cell.NCells,1)*Frac.Di_m)) .* ...
              max(Cell.ShieldsStress_i - ThetaCrit_i,0).^Inputs.Opt.ST.MPMexponent;
end
